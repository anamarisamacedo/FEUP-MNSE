...
import styles from "./JamOver.module.css";
...
function generateXML(song, settings, users) {
  ...
  for (const instrumentId of settings.instruments) {
    root = root
      .ele("score-part")
      .att("id", instrumentId)
      .ele("part-name", instruments[instrumentId].name)
      .up()
      // close score-part
      .up();
  }
    ...
    for (let i = 0; i < song.length; i += 1) {
      ...
      if (i === 0) {
        root = root
          ...
          .up()
          .ele("clef")
          .ele("sign", instruments[instrumentId].clef)
          .up()
          .ele("line", line)
          .up()
          .up()
      ...
      for (let j = 0; j < measure[0].length; j += 1) {
        ...
        if (instrumentNotes.length > 0) {
          ...
          for (let k = 0; k < instrumentNotes.length; k += 1) {
            ...
            if (instrumentNotes.length > 1 && k > 0)
              root = root.ele("chord").up();
            ...
            if (instruments[instrumentId] instanceof Tone.PolySynth) {
              ...
              root = root.ele("pitch").ele("step", pitch).up();
        ...
        <Grid container spacing={1} alignItems="stretch">
          ...
          <Grid item xs={2} style={{ minHeight: "80vh" }}>
            <Panel>
              ...
              <InputLabel id="selectExportFormat" style={{ color: "white " }}>
                Format
              </InputLabel>
              <Select
              ...
              >
                <MenuItem style={{ color: "black" }} value="image/png">
                  .png
                </MenuItem>
                <MenuItem style={{ color: "black" }} value="image/jpeg">
                  .jpeg
                </MenuItem>
              </Select>
              <Button
